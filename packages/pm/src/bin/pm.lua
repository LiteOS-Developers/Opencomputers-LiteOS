...
local function rm(...)
    local cmd = f("rm %s", table.concat({...}, " "))
    if (opts["dry-run"]) then
        print(cmd)
    else
        filesystem.remove(table.concat({...}, " "))
    end
end
...
return {
    main = function(args) 
        ...
        local args, opts = shell.parse(args)
        -- printHelp()
        if (mode == "info") then
            local manifest
        ...
        elseif (mode == "uninstall") then
            ...
            rm(f("%s", f("/etc/pm/info/%s.files", args[1])))
            if (opts.purge) then rm(f("%s", f("/etc/pm/info/%s.manifest", args[1]))) end
        else
            printHelp()
            os.exit(0)
        end --]]
